...
static void sendTelemetry()
{
  ...
  // Read JSON data from STM32Serial and construct JSON payload
  String receivedJson;
  while (STM32Serial.available())
  {
    uint8_t receivedParams[10];
    STM32Serial.readBytesUntil('\n', receivedParams, sizeof(receivedParams));

    char name[5];
    memcpy(name, receivedParams, 4);
    name[4] = '\0';
    uint16_t rollNo;
    memcpy(&rollNo, &receivedParams[4], sizeof(rollNo));
    uint8_t subject1 = receivedParams[7];
    uint8_t subject2 = receivedParams[8];
    ...
    // Assuming `mqtt_client` is already connected to Azure IoT Hub
...
//     receivedJson += receivedChar; // Append each character to the JSON string
//   }

//   // Convert the received JSON data to a C string
//   const char* telemetry_payload = receivedJson.c_str();
  
//   // Print the JSON data that is being sent
//   Serial.print("Sending JSON data: ");
...
//   // Convert the received data to a C string
//   const char* telemetry_payload = receivedChar.c_str();
